...
cat  ~/db.git/yurl_queue_httpcat.txt  | grep http  | perl -pe 's{[0-9]+::[0-9]+::http}{http}g' | sort | grep -i -v '\.jpg' | grep -i -v '\.gif' | grep -i -v '\.png' | tee ~/github/chrome_headless/tmp/urls_httpcat.txt >/dev/null
